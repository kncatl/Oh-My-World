...
public class ExprEvaluator {
    ...
    private static Object evalBinary(ExprNode.BinaryNode b, int x, int z, int ly) {
        Object left = eval(b.left(), x, z, ly);
        Object right = eval(b.right(), x, z, ly);

        BinaryOp op = b.op();
        if (op == BinaryOp.AND || op == BinaryOp.OR) {
            boolean lb = toBool(left);
            boolean rb = toBool(right);
            return op == BinaryOp.AND ? (lb && rb) : (lb || rb);
        }
        if (op == BinaryOp.EQ || op == BinaryOp.NE) {
            return compEqNe(op, left, right);
        }
        if (op == BinaryOp.LT || op == BinaryOp.GT || op == BinaryOp.LE || op == BinaryOp.GE) {
            double la = toDouble(left);
            double ra = toDouble(right);
            return switch (op) {
                case LT -> la < ra;
                case GT -> la > ra;
                case LE -> la <= ra;
        ...
        return switch (op) {
            ...
            case SUB -> la - ra;
            case MUL -> la * ra;
            case DIV -> (ra == 0 ? 0 : la / ra);
            case MOD -> (ra == 0 ? 0 : la % ra);
            default -> 0d;
        };
    }
    ...
    private static Object evalUnary(ExprNode.UnaryNode u, int x, int z, int ly) {
        ...
        return switch (u.op()) {
            case NOT -> !toBool(val);
            case NEG -> -toDouble(val);
        };
    }

    private static Object evalConditional(ExprNode.ConditionalNode c, int x, int z, int ly) {
        return toBool(eval(c.condition(), x, z, ly))
                ? eval(c.thenExpr(), x, z, ly)
                : eval(c.elseExpr(), x, z, ly);
    }

    private static Object evalFunc(ExprNode.FuncCallNode f, int x, int z, int ly) {
        List<Object> raw = new ArrayList<>();
        for (ExprNode arg : f.args()) {
            raw.add(eval(arg, x, z, ly));
        }
        return switch (f.name()) {
            case "floordiv" -> {
                int a = toInt(raw.get(0));
                int b = toInt(raw.get(1));
                yield (double) (b == 0 ? 0 : Math.floorDiv(a, b));
            }
            case "floormod" -> {
                int a = toInt(raw.get(0));
                int b = toInt(raw.get(1));
                yield (double) (b == 0 ? 0 : Math.floorMod(a, b));
            }
            case "abs"   -> Math.abs(toDouble(raw.get(0)));
            case "max"   -> Math.max(toDouble(raw.get(0)), toDouble(raw.get(1)));
            case "min"   -> Math.min(toDouble(raw.get(0)), toDouble(raw.get(1)));
            case "floor" -> Math.floor(toDouble(raw.get(0)));
            case "ceil"  -> Math.ceil(toDouble(raw.get(0)));
            case "round" -> (double) Math.round(toDouble(raw.get(0)));
            case "sign"  -> (double) Math.signum(toDouble(raw.get(0)));
            case "sqrt"  -> Math.sqrt(toDouble(raw.get(0)));
            case "pow"   -> Math.pow(toDouble(raw.get(0)), toDouble(raw.get(1)));
            case "exp"   -> Math.exp(toDouble(raw.get(0)));
            case "log"   -> Math.log(toDouble(raw.get(0)));
            case "log10" -> Math.log10(toDouble(raw.get(0)));
            case "sin"   -> Math.sin(toDouble(raw.get(0)));
            case "cos"   -> Math.cos(toDouble(raw.get(0)));
            case "tan"   -> Math.tan(toDouble(raw.get(0)));
            case "asin"  -> Math.asin(toDouble(raw.get(0)));
            case "acos"  -> Math.acos(toDouble(raw.get(0)));
            case "atan"  -> Math.atan(toDouble(raw.get(0)));
            case "todeg" -> Math.toDegrees(toDouble(raw.get(0)));
            case "torad" -> Math.toRadians(toDouble(raw.get(0)));
            case "rand" -> evalRand(raw, x, z, ly);
            case "randexcept" -> evalRandExcept(raw, x, z, ly);
            case "rng" -> evalRng(raw, x, z);
            case "smooth" -> evalSmooth(raw, x, z);
    ...
    private static Object evalRand(List<Object> raw, int x, int z, int ly) {
        ...
        for (Object arg : raw) {
            if (arg instanceof BlockState bs) states.add(bs);
        }
    ...
    private static Object evalRandExcept(List<Object> raw, int x, int z, int ly) {
        ...
        for (Object arg : raw) {
            if (arg instanceof BlockState bs) all.remove(bs);
        }
    ...
    private static Object evalRng(List<Object> raw, int x, int z) {
        double val = hashDouble(x, z);
        if (raw.size() >= 2) {
            double a = toDouble(raw.get(0));
            double b = toDouble(raw.get(1));
            return a + val * (b - a);
        }
        return val;
    }
    ...
    private static Object evalSmooth(List<Object> raw, int x, int z) {
        if (raw.size() >= 2) {
            double sx = toDouble(raw.get(0));
            double sz = toDouble(raw.get(1));
            return smoothNoise(x * sx, z * sz);
        }
    ...
    private static double smoothNoise(double x, double z) {
        ...
        fx = fx * fx * (3 - 2 * fx);
        fz = fz * fz * (3 - 2 * fz);
        double v00 = hashDouble(ix, iz);
        double v10 = hashDouble(ix + 1, iz);
        double v01 = hashDouble(ix, iz + 1);
        double v11 = hashDouble(ix + 1, iz + 1);
    ...
    public static BlockState evalToBlock(ExprNode node, int x, int z, int ly) {
        ...
        if (result instanceof BlockState bs) return bs;
