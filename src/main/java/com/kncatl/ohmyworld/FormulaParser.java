...
public class FormulaParser {
    ...
    public static List<Object> parse(String input) {
        ...
        for (String line : smartSplit(cleaned)) {
            ...
            try {
                int colonIdx = findColon(line); if (colonIdx < 0) continue;
                ...
                int eqIdx = rangePart.indexOf('='); if (eqIdx < 0) continue;
                if (!rangePart.substring(0, eqIdx).trim().equals("y")) continue;
                String valuePart = rangePart.substring(eqIdx + 1).trim();
                int yStart, yEnd; int dotsIdx = valuePart.indexOf("..");
                if (dotsIdx >= 0) { yStart = Integer.parseInt(valuePart.substring(0, dotsIdx).trim()); yEnd = Integer.parseInt(valuePart.substring(dotsIdx + 2).trim()); }
                else { yStart = yEnd = Integer.parseInt(valuePart); }
                if (exprPart.contains("*[")) {
                    ...
                    layers.add(new FormulaLayerDef(yStart, yEnd, new ExprParser(ExprLexer.tokenize(exprPart)).parse()));
                }
            } catch (Exception e) { LOGGER.error("FormulaParser: failed to parse: {}", line, e); }
        }
        return layers;
    }
    private static String[] smartSplit(String input) {
        ...
        parts.add(input.substring(start)); return parts.toArray(new String[0]);
    }
    private static List<CyclicLayerDef.Entry> parseCyclic(String exprPart) {
        List<CyclicLayerDef.Entry> entries = new ArrayList<>(); int pos = 0;
        while (pos < exprPart.length()) {
            ...
            int star = exprPart.indexOf('*', pos); if (star < 0) break;
            int t = Integer.parseInt(exprPart.substring(pos, star).trim()); pos = star + 1;
            if (pos >= exprPart.length() || exprPart.charAt(pos) != '[') break; pos++;
            int depth = 1, start = pos;
            while (pos < exprPart.length() && depth > 0) { char c = exprPart.charAt(pos); if (c == '[') depth++; else if (c == ']') depth--; if (depth > 0) pos++; }
            if (depth != 0) break;
            String inner = exprPart.substring(start, pos).trim(); pos++;
            entries.add(new CyclicLayerDef.Entry(t, new ExprParser(ExprLexer.tokenize(inner)).parse()));
        }
        return entries;
    }
    private static int findColon(String line) {
        int balance = 0; boolean inTernary = false;
