...
public class ExprEvaluator {
    ...
    private static Object evalBlockExpr(ExprNode.BlockExprNode be, int x, int z, int ly) {
        int required = 0;
        for (ExprNode.LetBinding lb : be.bindings()) if (lb.index() + 1 > required) required = lb.index() + 1;
        double[] slots = VAR_SLOTS.get();
        if (slots.length < required) VAR_SLOTS.set(slots = new double[required]);
        for (ExprNode.LetBinding lb : be.bindings()) {
            slots[lb.index()] = toDouble(eval(lb.value(), x, z, ly));
        }
    ...
    private static Object evalBinary(ExprNode.BinaryNode b, int x, int z, int ly) {
        BinaryOp op = b.op();
        if (op == BinaryOp.AND) return toBool(eval(b.left(), x, z, ly)) && toBool(eval(b.right(), x, z, ly));
        if (op == BinaryOp.OR) return toBool(eval(b.left(), x, z, ly)) || toBool(eval(b.right(), x, z, ly));
    ...
    private static Object evalRandExcept(List<Object> raw, int x, int z, int ly) {
        ...
        for (Object arg : raw) if (arg instanceof BlockState bs) all.remove(bs);
