...
public class OhMyWorldConfig {
    ...
    public static OhMyWorldConfig load() {
        ...
        if (Files.exists(CONFIG_PATH)) {
            try (Reader r = Files.newBufferedReader(CONFIG_PATH)) { instance = GSON.fromJson(r, OhMyWorldConfig.class); return instance; }
            catch (Exception e) { LOGGER.warn("Failed to load config, using defaults", e); }
        }
        instance = new OhMyWorldConfig(); save(); return instance;
    }
    public static void save() {
        try { Files.createDirectories(CONFIG_PATH.getParent()); try (Writer w = Files.newBufferedWriter(CONFIG_PATH)) { GSON.toJson(instance != null ? instance : new OhMyWorldConfig(), w); } }
